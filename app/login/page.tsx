...
import { ArrowLeft, LogIn, UserPlus, Sparkles, AlertCircle, CheckCircle2 } from 'lucide-react';
...
function LoginForm() {
  ...
  const [name, setName] = useState(() => {
    if (rawHandle) {
      return rawHandle.replace(/[-_.]/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
    }
    return '';
  });
  ...
  return (
    <div className="w-full max-w-md bg-[#0D0B18]/90 backdrop-blur-2xl border border-white/10 rounded-2xl p-8 shadow-[0_0_50px_rgba(138,63,252,0.2)] relative z-10">
      <div className="text-center mb-6 flex flex-col items-center">
        <Logo size="md" className="mb-3" />
        
        {/* Toggle Mode */}
        <div className="flex bg-white/5 p-1 rounded-xl border border-white/10 w-full mt-4">
          <button
            type="button"
        ...
        <p className="text-xs text-white/60 mt-3">
          {mode === 'login' 
            ? 'Acesse seu perfil inteligente, agenda e pedidos de shows.' 
            : 'Crie seu perfil profissional de DJ e comece a receber propostas.'}
        </p>

        {rawHandle && mode === 'signup' && (
          <div className="w-full mt-3 p-3 rounded-xl bg-gradient-to-r from-[#8A3FFC]/20 to-[#00D1FF]/20 border border-[#00D1FF]/40 flex items-center gap-2.5 text-xs text-white shadow-lg text-left">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
        {mode === 'signup' && (
          <div>
            ...
            <input 
              ...
              className="w-full h-10 bg-white/5 border border-white/10 rounded-lg px-3 text-sm text-white focus:outline-none focus:border-[#8A3FFC] transition-colors placeholder:text-white/20"
        ...
        <Button 
          ...
          className="w-full h-11 bg-gradient-to-r from-[#7C3AED] via-[#8A3FFC] to-[#00D1FF] hover:opacity-90 text-white font-semibold shadow-[0_0_25px_rgba(138,63,252,0.4)] mt-2"
        >
          {isSubmitting ? (
            'Processando...'
          ) : mode === 'login' ? (
      ...
      <div className="mt-6 pt-5 border-t border-white/10">
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button 
            ...
            className="w-full text-xs bg-white/5 border-white/10 hover:bg-white/10 hover:border-[#8A3FFC]/50 text-white flex items-center justify-center gap-1.5"
          >
